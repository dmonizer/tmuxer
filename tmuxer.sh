...
apply_tmux_settings() {
  ...
  # After mouse selection, copy the selected contents into global clipboard
...
_add_gsocket_host() {
  ...
  [[ -z "$gs_file" ]] && {
    echo "error: GSocket hosts file not configured" >&2
    printf 'Press Enter...'
    read -r _
    return
  }
  ...
  read -r desc
  [[ "$desc" == *'|'* ]] && {
    echo "error: description cannot contain '|'"
    printf 'Press Enter...'
    read -r _
    return
  }
  ...
  [[ "$secret" == *'|'* ]] && {
    echo "error: secret cannot contain '|'"
    printf 'Press Enter...'
    read -r _
    return
  }
...
# F10: toggle raw mode (pty.spawn connections) — was F3
popup_f10() {
  local raw
  raw=$(tmux show-options -wv @raw_mode 2>/dev/null)
  if [[ "$raw" == "1" ]]; then
    stty sane < "$(tmux display-message -p '#{pane_tty}')"
    tmux set-option -w @raw_mode 0
    tmux display-message "raw mode OFF — cooked restored"
  else
    stty raw -echo < "$(tmux display-message -p '#{pane_tty}')"
    tmux set-option -w @raw_mode 1
    tmux display-message "raw mode ON — pty.spawn active"
  fi
}
...
handle_connection() {
  ...
  win_id=$(tmux new-window -P -F "#{window_id}" -n "$remote_ip" \
    "echo '[*] connection from $remote_ip — [F5] commands'; echo '[F10] raw mode toggle'; trap 'stty sane; rm -f $in $out' EXIT; cat <$in & cat >$out; wait")
  ...
  if [[ $pty_detected -eq 1 ]]; then
    ...
    printf '[*] PTY detected — raw mode ON (F10 to toggle)\n' >"$new_pane_tty"
  else
    printf '[*] No PTY — use pty.spawn + F10 to upgrade\n' >"$new_pane_tty"
  fi
...
popup)
  ...
  *)
    echo "unknown popup action: $POPUP_ACTION" >&2
    exit 1
    ;;
  esac
  ;;
